// ============================================================
// RESTBR SUPER ADMIN PLAN V2.0
// Changes Basic / Pro / Premium / Internal for existing restaurants.
// Requires RESTBR-SUBSCRIPTIONS-V1.sql.
// ============================================================
(() => {
  'use strict';

  let sb=null;
  const state={restaurantId:null,name:'',currentPlan:'basic'};
  ...
  async function open(id,name){
    state.restaurantId=id;state.name=name||'المطعم';
    ...
    try{
      ...
      const {data,error}=await c.from('subscriptions').select('plan,starts_at').eq('restaurant_id',id).order('starts_at',{ascending:false}).limit(1);
  ...
  async function save(){
    ...
    try{
      const c=await client();const {error}=await c.rpc('admin_set_restaurant_plan',{p_restaurant_id:state.restaurantId,p_plan:plan});if(error)throw error;
      state.currentPlan=plan;msg('تم تحديث الخطة ✓','ok');
      setTimeout(()=>{close();document.getElementById('refreshBtn')?.click();},450);
    }catch(error){const text=String(error?.message||error);msg(/admin_set_restaurant_plan.*does not exist/i.test(text)?'شغّل RESTBR-SUBSCRIPTIONS-V1.sql في Supabase أولاً.':text,'err');}
  ...
  function boot(){styles();ensureModal();inject();const list=$('restaurantList');if(list)new MutationObserver(inject).observe(list,{childList:true,subtree:true});console.log('✅ RESTBR Super Admin Plan V2.0 ready');}
