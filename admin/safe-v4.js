(() => {
  ...
  async function getPlatformConfig() {
    ...
    if (!response.ok || !data?.ok || !data?.supabase_url || !data?.publishable_key) {
      throw new Error(data?.message || 'تعذر تحميل إعدادات المنصة.');
    }
    return data;
  }
  ...
  async function boot() {
    try {
      ...
      state.client = window.supabase.createClient(config.supabase_url, config.publishable_key, {
        auth: {
          persistSession: true,
  ...
  async function loadDashboard() {
    ...
    try {
      ...
      for (const result of [restaurantsResult, domainsResult, subscriptionsResult, membersResult]) {
      ...
      render();
      els.lastUpdated.textContent = `آخر تحديث: ${new Date().toLocaleTimeString('ar-IQ', { hour: '2-digit', minute: '2-digit' })} • Safe V4.1`;
    } catch (error) {
      console.error(error);
  ...
  function ownersFor(restaurantId) {
    return state.members.filter(m => m.restaurant_id === restaurantId && m.is_active && m.role === 'owner');
  }

  function ownerUrlFor(restaurant) {
    const domain = domainFor(restaurant.id);
    const hostname = domain?.hostname || `${restaurant.slug}.restbr.com`;
    const url = new URL(`https://${hostname}/owner/`);
    url.searchParams.set('tenant', restaurant.slug);
    return url.toString();
  }

  function statusLabel(status) {
    return ({ active: 'نشط', suspended: 'موقوف', draft: 'مسودة', archived: 'مؤرشف' })[status] || status;
  }
      ...
      return `
        <article class="restaurant-card" data-id="${esc(r.id)}">
          <div class="restaurant-main">
            ...
            <div style="margin-top:8px"><span class="status-pill ${ownerClass}">Owner ${owners.length}</span></div>
          </div>
          ...
          <div class="card-actions">
            ...
            <button class="mini-btn" data-action="assign-owner" data-id="${esc(r.id)}" data-owner-url="${esc(ownerUrl)}">ربط مالك</button>
  ...
  async function changeStatus(id, status, button) {
    ...
    try {
      const { error } = await state.client.from('restaurants').update({ status }).eq('id', id);
      if (error) throw error;
  ...
  async function assignOwner(button) {
    const email = window.prompt('اكتب بريد حساب صاحب المطعم الموجود في RESTBR Auth:');
    if (!email) return;
    const cleanEmail = email.trim().toLowerCase();
    if (!cleanEmail || !cleanEmail.includes('@')) {
      toast('البريد غير صالح.');
      return;
    }

    button.disabled = true;
    try {
      const { error } = await state.client.rpc('admin_assign_restaurant_member', {
        p_restaurant_id: button.dataset.id,
        p_user_email: cleanEmail,
        p_role: 'owner'
      });
      if (error) throw error;
      toast('تم ربط حساب المالك بالمطعم ✅');
      await loadDashboard();
    } catch (error) {
      console.error(error);
      const text = String(error?.message || error);
      if (/auth user not found/i.test(text)) {
        const openSignup = window.confirm('هذا البريد ما عنده حساب RESTBR بعد. افتح صفحة Owner الآن حتى تنشئ الحساب؟');
        if (openSignup) window.open(button.dataset.ownerUrl, '_blank', 'noopener');
      } else {
        toast(text || 'تعذر ربط المالك.');
      }
    } finally {
      button.disabled = false;
    }
  ...
  async function handleRestaurantAction(event) {
    ...
    if (action === 'open' || action === 'owner') {
      window.open(button.dataset.url, '_blank', 'noopener');
      return;
    }

    if (action === 'assign-owner') {
      await assignOwner(button);
      return;
    }
