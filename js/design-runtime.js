...
// Tenant-specific visual settings
// ==========================================
// This runtime only applies keys that exist in ui_design_settings.
// If a restaurant has no design keys, its current appearance stays unchanged.
...
(() => {
  ...
  function apply(settings = {}) {
    ...
    // New tenant-specific theme controls.
    ...
    const overlay = alpha(ui.background_overlay_opacity);
    if (overlay !== null) css('--restbr-bg-overlay', `rgba(0,0,0,${overlay})`);
    ...
    const injected = document.getElementById('restbrDesignSystemV1') || document.createElement('style');
    injected.id = 'restbrDesignSystemV1';
    injected.textContent = `
      :root {
        ...
        --restbr-bg-overlay: rgba(0,0,0,.08);
      }
      ...
      .sm-bg-overlay {
        background: var(--restbr-bg-overlay) !important;
      }
      ...
      .sm-footer-card {
        ...
        backdrop-filter: blur(var(--restbr-card-blur)) saturate(125%) !important;
        -webkit-backdrop-filter: blur(var(--restbr-card-blur)) saturate(125%) !important;
    ...
    `;
    if (!injected.parentNode) document.head.appendChild(injected);
  ...
  async function boot() {
    try {
      ...
      console.warn('RESTBR Design System V1 could not apply:', error);
    }
  }
