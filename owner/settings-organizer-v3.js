// ============================================================
// RESTBR OWNER SETTINGS ORGANIZER V3.0
// Topic-based groups + per-setting reset + per-section reset.
// Resets edit the form only; the owner presses the existing Save button so
// unrelated unsaved changes are never committed accidentally.
// ============================================================
(() => {
  'use strict';
  ...
  const groups={
    ...
    design:[
      {id:'rbGroupTheme',title:'🎨 الثيم والألوان',desc:'هوية الثيم، لون Accent، لون الزجاج ومؤثر الخلفية.',fields:['dAccent','dGlassColor','dBackgroundEffect']},
      {id:'rbGroupCards',title:'🍽 كروت المنتجات',desc:'الشفافية، Blur، الارتفاع، الاستدارة ونسبة الصورة.',fields:['dOpacity','dBlur','dCardHeight','dRadius','dImagePercent']},
      {id:'rbGroupFooter',title:'🪟 الفوتر',desc:'زجاج الفوتر مستقل عن كروت المنتجات.',fields:['rbFooterOpacity','rbFooterBlur','rbFooterColor']}
    ]
  };
  ...
  async function resolveRestaurantName(){
    ...
    try{
      ...
      if(host==='hamodybr.github.io'){
        const slug=String(new URLSearchParams(location.search).get('tenant')||'').trim().toLowerCase();q=await sb.from('restaurants').select('name').eq('slug',slug).maybeSingle();
      }else q=await sb.from('restaurant_domains').select('restaurants!inner(name)').eq('hostname',host).eq('status','active').eq('is_verified',true).maybeSingle();
      if(!q.error)state.restaurantName=q.data?.restaurants?.name||q.data?.name||'';
    }catch(_){}
  ...
  async function resetField(id){
    const el=$(id);if(!el||!(id in defaults))return;await resolveRestaurantName();let value=defaults[id];if(typeof value==='function')value=value();
    if(el.type==='checkbox')el.checked=Boolean(value);else if(el.type==='file')el.value='';else el.value=String(value??'');dispatch(el);showPending(el);
  }
  function showPending(el){
    const view=el.closest('#view-design')?'designMsg':'settingsMsg',msg=$(view);if(msg){msg.textContent='تم إرجاع القيمة الافتراضية داخل النموذج. اضغط حفظ لتثبيت التغيير.';msg.className='status';}
  }

  function addFieldReset(id){
    const el=$(id);if(!el||!(id in defaults)||el.dataset.rbResetReady==='1')return;const wrap=wrapperFor(el);if(!wrap)return;el.dataset.rbResetReady='1';wrap.classList.add('rb-reset-wrap');const b=document.createElement('button');b.type='button';b.className='rb-reset-one';b.title='إعادة هذا الإعداد للافتراضي';b.textContent='↺';b.onclick=e=>{e.preventDefault();e.stopPropagation();void resetField(id);};wrap.appendChild(b);
  }

  function sectionMarkup(group){
    const section=document.createElement('section');section.id=group.id;section.className='rb-org-section';section.innerHTML=`<div class="rb-org-head"><div><h4>${group.title}</h4><p>${group.desc}</p></div><button type="button" class="rb-org-reset">↺ إعادة ضبط القسم</button></div><div class="rb-org-grid"></div><div class="rb-org-extra"></div><div class="rb-org-note">زر ↺ يرجع القيمة داخل النموذج فقط. استخدم زر الحفظ الموجود في الصفحة لتثبيت التغيير.</div>`;section.querySelector('.rb-org-reset').onclick=()=>group.fields.forEach(id=>void resetField(id));return section;
  }

  function moveFields(group,section){
    const grid=section.querySelector('.rb-org-grid');group.fields.forEach(id=>{const el=$(id),wrap=wrapperFor(el);if(wrap&&wrap.closest('.rb-org-section')!==section)grid.appendChild(wrap);addFieldReset(id);});
  }

  function hideLegacyEmpty(view){
    view.querySelectorAll(':scope > .panel > .grid2,:scope > .panel > .grid3,:scope > .panel > .panel').forEach(node=>{if(!node.querySelector('input,select,textarea,button'))node.style.display='none';});
  }

  function organizeSettings(){
    const view=$('view-settings'),panel=view?.querySelector(':scope > .panel');if(!panel)return;
    let stack=$('rbSettingsGroups');if(!stack){stack=document.createElement('div');stack.id='rbSettingsGroups';stack.className='rb-org-stack';const bar=$('saveSettingsBtn')?.closest('.savebar');bar?.before(stack);}
    groups.settings.forEach(group=>{let section=$(group.id);if(!section){section=sectionMarkup(group);stack.appendChild(section);}moveFields(group,section);});

    const operations=$('rbGroupOperations')?.querySelector('.rb-org-extra');
    const contact=$('rbGroupContact')?.querySelector('.rb-org-extra');
    const boxes=[...document.querySelectorAll('#rbv2Operations>.rbv2-box')];
    boxes.forEach(box=>{const text=box.textContent||'';if(/الموقع|روابط التواصل/.test(text))contact?.appendChild(box);else operations?.appendChild(box);});
    const rbv2=$('rbv2Operations');if(rbv2&&!rbv2.querySelector('.rbv2-box'))rbv2.style.display='none';
  ...
  function boot(){scan();new MutationObserver(()=>requestAnimationFrame(scan)).observe(document.body,{childList:true,subtree:true});console.log('✅ RESTBR Owner Settings Organizer V3.0 ready');}
