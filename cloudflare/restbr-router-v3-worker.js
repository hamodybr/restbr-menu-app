// RESTBR Router V3
// Dynamic multi-restaurant router + public menu bootstrap API.
//
// Required Cloudflare Worker bindings:
//   SUPABASE_URL
//   SUPABASE_PUBLISHABLE_KEY
//
// Never place a service_role / sb_secret key in this public router.
...
function mergeLegacySettings(settings, restaurant) {
  const branding =
  ...
  return {
    ...
    // Stable tenant identity fields for the shared menu core.
...
async function getMenuBootstrap(route, env) {
  ...
  const settings = mergeLegacySettings(settingsRows?.[0] || {}, restaurant);
...
async function trackMenuEvent(request, route, env) {
  ...
  const response = await fetch(`${supabaseBase(env)}/rest/v1/rpc/track_menu_event`, {
    method: "POST",
    headers: supabaseHeaders(env, {
...
async function proxyRestaurant(request, route) {
  ...
  const requestedPath = incomingUrl.pathname === "/" ? "/" : incomingUrl.pathname;

  const targetUrl = new URL(
    `${basePath}${requestedPath}${incomingUrl.search}`,
    `${origin}/`
  );
  ...
  headers.set("x-restbr-restaurant", restaurant.slug);
  headers.set("x-restbr-router", "v3");
  ...
  async fetch(request, env) {
    try {
      ...
      if (hostname === PLATFORM_ROOT || hostname === `www.${PLATFORM_ROOT}`) {
        return json(
          { ok: false, error: "platform_root_not_handled_by_restaurant_router" },
          404
        );
      }
      ...
      if (url.pathname === "/_restbr/bootstrap") {
        ...
        const data = await getMenuBootstrap(route, env);
      ...
      if (url.pathname === "/_restbr/track") {
        return trackMenuEvent(request, route, env);
      }

      return await proxyRestaurant(request, route);
    } catch (error) {
      console.error("RESTBR Router V3 error:", error);
      return json(
        {
          ok: false,
