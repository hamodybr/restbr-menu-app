-- ============================================================
-- RESTBR BACKUP / RESTORE V1
-- Atomic Owner/Manager restore + atomic menu clear.
-- Every RPC verifies tenant management permission before modifying data.
-- A failed restore rolls back the entire PostgreSQL statement automatically.
-- ============================================================
...
begin
  ...
  if p_settings is null or jsonb_typeof(p_settings) <> 'object' then
    raise exception 'invalid restaurant settings backup';
  end if;
  ...
  if jsonb_typeof(coalesce(p_categories,'[]'::jsonb)) <> 'array'
    ...
    raise exception 'invalid menu backup arrays';
  end if;

  -- Dependent rows first. The function call is a single database transaction;
  -- any exception after this point rolls all deletes/inserts back.
  ...
  select
    x.id,p_restaurant_id,x.slug,x.name_ar,x.name_ku,x.name_en,
    coalesce(x.sort_order,0),coalesce(x.is_active,true),coalesce(x.is_visible,true),
    coalesce(x.availability_schedule_enabled,false),x.available_from,x.available_to,
    coalesce(x.metadata,'{}'::jsonb),coalesce(x.created_at,now()),now()
  from jsonb_to_recordset(coalesce(p_categories,'[]'::jsonb)) as x(
    id uuid,restaurant_id uuid,slug text,name_ar text,name_ku text,name_en text,
    sort_order integer,is_active boolean,is_visible boolean,
    availability_schedule_enabled boolean,available_from time,available_to time,
    metadata jsonb,created_at timestamptz,updated_at timestamptz
  );
  get diagnostics v_category_count = row_count;

  insert into public.products(
    id,restaurant_id,category_id,name_ar,name_ku,name_en,
    description_ar,description_ku,description_en,image_url,base_price,sort_order,
    is_available,is_visible,availability_schedule_enabled,available_from,available_to,
    metadata,created_at,updated_at
  )
  select
    x.id,p_restaurant_id,x.category_id,x.name_ar,x.name_ku,x.name_en,
    x.description_ar,x.description_ku,x.description_en,x.image_url,
    coalesce(x.base_price,0),coalesce(x.sort_order,0),
    coalesce(x.is_available,true),coalesce(x.is_visible,true),
    coalesce(x.availability_schedule_enabled,false),x.available_from,x.available_to,
    coalesce(x.metadata,'{}'::jsonb),coalesce(x.created_at,now()),now()
  from jsonb_to_recordset(coalesce(p_products,'[]'::jsonb)) as x(
    id uuid,restaurant_id uuid,category_id uuid,name_ar text,name_ku text,name_en text,
    description_ar text,description_ku text,description_en text,image_url text,
    base_price numeric,sort_order integer,is_available boolean,is_visible boolean,
    availability_schedule_enabled boolean,available_from time,available_to time,
    metadata jsonb,created_at timestamptz,updated_at timestamptz
  );
  ...
  insert into public.product_options(
    id,restaurant_id,product_id,name_ar,name_ku,name_en,price,sort_order,is_active,
    metadata,created_at,updated_at
  )
  select
    x.id,p_restaurant_id,x.product_id,x.name_ar,x.name_ku,x.name_en,
    coalesce(x.price,0),coalesce(x.sort_order,0),coalesce(x.is_active,true),
    coalesce(x.metadata,'{}'::jsonb),coalesce(x.created_at,now()),now()
  from jsonb_to_recordset(coalesce(p_options,'[]'::jsonb)) as x(
    id uuid,restaurant_id uuid,product_id uuid,name_ar text,name_ku text,name_en text,
    price numeric,sort_order integer,is_active boolean,metadata jsonb,
    created_at timestamptz,updated_at timestamptz
  );
  get diagnostics v_option_count = row_count;

  -- Restore only tenant settings. Domains, restaurant identity, membership and
  -- subscription stay platform-controlled and are intentionally not overwritten.
  ...
  set
    ...
    languages = case
      when jsonb_typeof(p_settings -> 'languages')='array' then
  ...
  if not found then
    raise exception 'restaurant settings row not found';
  end if;
