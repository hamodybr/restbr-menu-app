...
7. أخيرًا شغّل `supabase/migrations/20260911173100_order_hours_guard.sql` حتى تعيد قاعدة البيانات نفسها التحقق من أوقات عمل المطعم اليومية/الأسبوعية بتوقيت `Asia/Baghdad` عند إنشاء أي طلب.

ملفات الـmigration مكتوبة لتكون idempotent قدر الإمكان ويمكن إعادة تشغيلها عند التحقق من البيئة. حافظ على ترتيبها: الألوان → تثبيت الطلبات → حارس أوقات الطلبات، لأن نظام الطلبات يحفظ Snapshot للون المختار والحارس يعمل على إنشاء الطلب النهائي.

الـbootstrap ينشئ الجداول الأساسية وRLS والإحصائيات والأسعار والخصومات وأوقات التوفر وحاوية `menu-images` والإعدادات. Migration الألوان تضيف `product_colors`، وMigration الطلبات تضيف `submit_order` الآمن وحقول idempotency و`delivery_fee`، وMigration الحارس تمنع تجاوز أوقات عمل المطعم حتى بطلب API مصطنع خارج الواجهة.
